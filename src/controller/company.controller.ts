...
async function GenerateCompaniesRecommendation(
	...
	try {
		const userMarketArea = "Tecnologia da Informação"
		const userTechnologies = ["Machine Learning", "Cloud Computing"]

		const arg1 = userMarketArea
		const arg2 = userTechnologies.join(",")

		const resultado = await executarScriptPython(arg1, arg2)
...
async function executarScriptPython(
	arg1: string,
	arg2: string
): Promise<string[]> {
...
export { Company, CreateCompany, GenerateCompaniesRecommendation }
